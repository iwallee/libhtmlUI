...
class WebFrameClientImpl
    ...
    , public base::SupportsWeakPtr<WebFrameClientImpl>
{
public:
    // May return null.
    virtual WebPlugin* createPlugin(WebFrame*, const WebPluginParams&) ;

    // May return null.
    virtual WebMediaPlayer* createMediaPlayer(WebFrame*, const WebURL&, WebMediaPlayerClient*) ;
    ...
    virtual WebServiceWorkerProvider* createServiceWorkerProvider(WebFrame*, WebServiceWorkerProviderClient*) ;

    // May return null.
    virtual WebWorkerPermissionClientProxy* createWorkerPermissionClientProxy(WebFrame*) ;
    ...
    virtual WebCookieJar* cookieJar(WebFrame*) ;
    ...
    virtual WebFrame* createChildFrame(WebFrame* parent, const WebString& frameName) ;
    ...
    virtual WebString userAgentOverride(WebFrame*, const WebURL& url);

    // Asks the embedder what value the network stack will send for the DNT
    // header. An empty string indicates that no DNT header will be send.
    virtual WebString doNotTrackValue(WebFrame*);
    ...
    virtual bool allowWebGL(WebFrame*, bool defaultValue);
